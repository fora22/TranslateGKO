...
파이선 언어에 적용할 수 있는 프로그래밍 방법에는 절차적, 함수형, 그리고 객체지향으로 세 가지 유형이 있다.
...
고차함수는 다른 함수를 매개변수로 사용하거나 함수를 반환하는 함수이다.
...
| 연산자 |  예시   | 설명                                              |
| :------: | ------------ | --------------------------------------------------------- |
|   `@`    | `@decorator` | `@decorator` 는 `func = decorator(func)`을 대체한다. |
...
수정될 함수에 가장 가까운 데코레이터가 우선적으로 적용된다. 그러므로, `function()` 함수는 먼저 `@decorator1` 다음 `@decorator0`으로 순차적으로 적용된다.